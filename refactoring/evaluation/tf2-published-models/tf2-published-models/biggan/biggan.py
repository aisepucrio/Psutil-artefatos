...
class SelfAttn(tf.keras.layers.Layer):
    ...
    def call(self, x: tf.Tensor) -> tf.Tensor:
        ...
        attn_g = tf.transpose(attn_g, (0, 2, 1))
...
class BigGANBatchNorm(tf.keras.layers.Layer):
    ...
    def _get_running_stats(self, truncation: float):
    ...
    def _conditional_batch_norm(self, x, running_mean, running_var, condition_vector):
    ...
    def _non_conditional_batch_norm(self, x, running_mean, running_var):
    ...
    def call(
        ...
        running_mean, running_var = self._get_running_stats(truncation)
        if self.conditional:
            out = self._conditional_batch_norm(x, running_mean, running_var, condition_vector)
        else:
            out = self._non_conditional_batch_norm(x, running_mean, running_var)
        return out
...
class GenBlock(tf.keras.layers.Layer):
    ...
    def _upsample_and_drop_channels(self, x0, x):
    ...
    def call(
        ...
        x = self.relu(x)
        x = self.conv_3(x)
        x0 = self._upsample_and_drop_channels(x0, x)
        out = x + x0
        return out
...
def _get_batchnorm_weights(batch_pnt, mapping, prefix):
    for name in ("offset", "scale"):
        sub_module_str = prefix + name + "/"
...
def _get_conv_weights(conv_pnt, mapping, prefix):
    mapping.update(
        {
            prefix + "b/ema_0.9999": conv_pnt.layer.bias,
            prefix + "w/ema_0.9999": conv_pnt.layer.kernel,
            prefix + "u0": conv_pnt.u,
        }
    )

def _add_genblock_weights(model, config, mapping, i, model_layer_idx, layer_str, layer_pnt):
    for j in range(4):
        batch_str = layer_str + (
            "BatchNorm_%d/" % j if j > 0 else "BatchNorm/"
        )
        batch_pnt = getattr(layer_pnt, "bn_%d" % j)
        _get_batchnorm_weights(batch_pnt, mapping, batch_str)
    for j in range(4):
        conv_str = layer_str + "conv%d/" % j
        conv_pnt = getattr(layer_pnt, "conv_%d" % j)
        _get_conv_weights(conv_pnt, mapping, conv_str)
    return model_layer_idx + 1

def _add_attention_weights(model, config, mapping, layer_str, layer_pnt):
...
def weights_mapping(model, config: BigGANConfig) -> Dict[str, tf.Variable]:
    ...
    for i, (up, in_channels, out_channels) in enumerate(config.layers):
        ...
        model_layer_idx = _add_genblock_weights(model, config, mapping, i, model_layer_idx, layer_str, layer_pnt)
    layer_str = "Generator/attention/"
    layer_pnt = model.generator.layers[config.attention_layer_position]
    mapping = _add_attention_weights(model, config, mapping, layer_str, layer_pnt)
...
class BigGAN(tf.keras.Model):
    ...
    @classmethod
    def _get_config_and_weights(cls, config_name: str) -> Tuple[BigGANConfig, str]:
        pretrained_urls = {
    ...
    def from_pretrained(cls, config_name: str) -> BigGAN:
        config, weights_path = cls._get_config_and_weights(config_name)
        weights_folder = os.path.join(os.path.dirname(__file__), config_name)
...
tracker = EmissionsTracker(project_name=r"C:\Users\PUC\Documents\AISE\ecosustain-replication-study\artefatos\tf2-published-models\biggan\biggan.py")
tracker.start()
