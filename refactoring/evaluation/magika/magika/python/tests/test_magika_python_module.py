...
# Start CodeCarbon tracker with project name "C:\Users\PUC\Documents\AISE\ecosustain-replication-study\artefatos\magika\python\tests\test_magika_python_module.py" and collect initial system metrics
tracker = EmissionsTracker(project_name=r"C:\Users\PUC\Documents\AISE\ecosustain-replication-study\artefatos\magika\python\tests\test_magika_python_module.py")
tracker.start()
...
def _check_prediction_mode_results(
    m: Magika, expected_label: ContentTypeLabel, score: float, expected_overwrite_reason: OverwriteReason
) -> None:
    result = m._get_output_label_from_dl_label_and_score(
        ContentTypeLabel.PYTHON, score
    )
    assert result == (expected_label, expected_overwrite_reason)


def test_magika_module_with_different_prediction_modes() -> None:
    model_dir = utils.get_default_model_dir()
    m_best_guess = Magika(model_dir=model_dir, prediction_mode=PredictionMode.BEST_GUESS)
    m_medium_confidence = Magika(model_dir=model_dir, prediction_mode=PredictionMode.MEDIUM_CONFIDENCE)
    m_high_confidence = Magika(model_dir=model_dir, prediction_mode=PredictionMode.HIGH_CONFIDENCE)
    m_default = Magika(model_dir=model_dir)

    _check_prediction_mode_results(m_best_guess, ContentTypeLabel.PYTHON, 0.01, OverwriteReason.NONE)
    _check_prediction_mode_results(m_best_guess, ContentTypeLabel.PYTHON, 0.40, OverwriteReason.NONE)
    _check_prediction_mode_results(m_best_guess, ContentTypeLabel.PYTHON, 0.60, OverwriteReason.NONE)
    _check_prediction_mode_results(m_best_guess, ContentTypeLabel.PYTHON, 0.99, OverwriteReason.NONE)

    _check_prediction_mode_results(m_medium_confidence, ContentTypeLabel.TXT, 0.01, OverwriteReason.LOW_CONFIDENCE)
    _check_prediction_mode_results(m_medium_confidence, ContentTypeLabel.TXT, m_medium_confidence._model_config.medium_confidence_threshold - 0.01, OverwriteReason.LOW_CONFIDENCE)
    _check_prediction_mode_results(m_medium_confidence, ContentTypeLabel.PYTHON, 0.60, OverwriteReason.NONE)
    _check_prediction_mode_results(m_medium_confidence, ContentTypeLabel.PYTHON, 0.99, OverwriteReason.NONE)

    high_confidence_threshold = m_high_confidence._model_config.thresholds.get(
        ContentTypeLabel.PYTHON, m_high_confidence._model_config.medium_confidence_threshold
    )
    _check_prediction_mode_results(m_high_confidence, ContentTypeLabel.TXT, 0.01, OverwriteReason.LOW_CONFIDENCE)
    _check_prediction_mode_results(m_high_confidence, ContentTypeLabel.TXT, high_confidence_threshold - 0.01, OverwriteReason.LOW_CONFIDENCE)
    _check_prediction_mode_results(m_high_confidence, ContentTypeLabel.PYTHON, high_confidence_threshold + 0.01, OverwriteReason.NONE)
    _check_prediction_mode_results(m_high_confidence, ContentTypeLabel.PYTHON, 0.99, OverwriteReason.NONE)

    high_confidence_threshold = m_default._model_config.thresholds.get(
        ContentTypeLabel.PYTHON, m_default._model_config.medium_confidence_threshold
    )
    _check_prediction_mode_results(m_default, ContentTypeLabel.TXT, 0.01, OverwriteReason.LOW_CONFIDENCE)
    _check_prediction_mode_results(m_default, ContentTypeLabel.TXT, high_confidence_threshold - 0.01, OverwriteReason.LOW_CONFIDENCE)
    _check_prediction_mode_results(m_default, ContentTypeLabel.PYTHON, high_confidence_threshold + 0.01, OverwriteReason.NONE)
    _check_prediction_mode_results(m_default, ContentTypeLabel.PYTHON, 0.99, OverwriteReason.NONE)
...
def test_magika_module_overwrite_reason() -> None:
    ...
    _check_prediction_mode_results(m_high, ContentTypeLabel.PYTHON, python_high_confidence_threshold + 0.01, OverwriteReason.NONE)
    _check_prediction_mode_results(m_high, ContentTypeLabel.TXT, python_high_confidence_threshold - 0.01, OverwriteReason.LOW_CONFIDENCE)

    _check_prediction_mode_results(m_medium, ContentTypeLabel.PYTHON, medium_confidence_threshold + 0.01, OverwriteReason.NONE)
    _check_prediction_mode_results(m_medium, ContentTypeLabel.TXT, medium_confidence_threshold - 0.01, OverwriteReason.LOW_CONFIDENCE)

    _check_prediction_mode_results(m_best, ContentTypeLabel.PYTHON, medium_confidence_threshold + 0.01, OverwriteReason.NONE)
    _check_prediction_mode_results(m_best, ContentTypeLabel.PYTHON, medium_confidence_threshold - 0.01, OverwriteReason.NONE)
    ...
    for overwrite_map_ct_key in sorted(m_high._model_config.overwrite_map.keys()):
        ...
        _check_prediction_mode_results(m_high, overwrite_map_ct_value, overwrite_map_ct_high_confidence_threshold + 0.01, OverwriteReason.OVERWRITE_MAP)
        _check_prediction_mode_results(m_high, ContentTypeLabel.TXT if is_overwrite_map_ct_target_text else ContentTypeLabel.UNKNOWN, overwrite_map_ct_high_confidence_threshold - 0.01, OverwriteReason.LOW_CONFIDENCE)
    ...
    for generic_ct in [ContentTypeLabel.TXT, ContentTypeLabel.UNKNOWN]:
        ...
        _check_prediction_mode_results(m_high, generic_ct, generic_type_high_confidence_threshold - 0.01, OverwriteReason.NONE)
        _check_prediction_mode_results(m_medium, generic_ct, medium_confidence_threshold - 0.01, OverwriteReason.NONE)
