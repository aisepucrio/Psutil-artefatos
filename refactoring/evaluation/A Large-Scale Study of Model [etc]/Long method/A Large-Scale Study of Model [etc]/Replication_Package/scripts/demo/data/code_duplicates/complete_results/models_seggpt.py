...
class SegGPT(nn.Module):
    def __init__(
        ...
        self.pos_embed = self._init_pos_embed(use_abs_pos, pretrain_img_size)
    ...
    def _init_pos_embed(self, use_abs_pos, pretrain_img_size):
        if use_abs_pos:
            ...
            return nn.Parameter(torch.zeros(1, num_positions, self.embed_dim), requires_grad=True)
        return None
...
def seggpt_vit_large_patch16_input896x448(**kwargs):
    model = SegGPT(
        ...
        window_block_indexes=(
            list(range(0, 2)) + list(range(3, 5)) + list(range(6, 8)) + list(range(9, 11)) + list(range(12, 14)),
            list(range(15, 17)),
            list(range(18, 20)),
            list(range(21, 23)),
        ),
