...
tracker = EmissionsTracker(project_name=r"C:\Users\guicu\OneDrive\Documentos\prog\aise\artifact\artifacts\A Large-Scale Study of Model Integration in ML-Enabled Software Systems\Replication_Package\scripts\demo\data\code_duplicates\complete_results\models_painter.py")
tracker.start()
...
class Painter(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = self._create_positional_embedding(pretrain_img_size, embed_dim, use_abs_pos)
    ...
    def _create_positional_embedding(self, pretrain_img_size, embed_dim, use_abs_pos):
        if use_abs_pos:
            num_patches = (pretrain_img_size // self.patch_size) * (pretrain_img_size // self.patch_size)
            num_positions = (num_patches + 1) if self.pretrain_use_cls_token else num_patches
            return nn.Parameter(torch.zeros(1, num_positions, embed_dim), requires_grad=True)
        return None

    def _create_transformer_blocks(
        self,
        depth,
        embed_dim,
        num_heads,
        ...
        for i in range(depth):
            block = Block(
                ...
                input_size=(img_size[0] // self.patch_size, img_size[1] // self.patch_size),
            )
    ...
    def forward_loss(self, pred, tgts, mask, valid):
        ...
        inds_ign = ((tgts * imagenet_std + imagenet_mean) * (1 - 1.0 * mask)).sum((1, 2, 3)) < 100 * 3
        if inds_ign.sum() > 0:
            valid[inds_ign] = 0.0
        mask = mask * valid
        target = tgts
        ...
        loss = (loss * mask).sum() / (mask.sum() + 1e-2)
        return loss
