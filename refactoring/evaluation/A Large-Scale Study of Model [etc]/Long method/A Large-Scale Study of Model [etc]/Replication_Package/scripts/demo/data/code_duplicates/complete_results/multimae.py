...
class MultiMAE(nn.Module):
    ...
    def __init__(self,
        ...
        self._init_adapters(dim_tokens)
        self.apply(self._init_weights)
        self._init_linear_weights()

    def _init_adapters(self, dim_tokens):
        for adapter in self.input_adapters.values():
            adapter.init(dim_tokens=dim_tokens)
        if self.output_adapters is not None:
            for adapter in self.output_adapters.values():
    ...
    def _encode_input(self, x):
        return {domain: self.input_adapters[domain](tensor) for domain, tensor in x.items() if domain in self.input_adapters}

    def _mask_tokens(self, input_task_tokens, num_encoded_tokens, alphas, sample_tasks_uniformly):
    ...
    def _handle_predefined_masks(self, input_task_tokens, task_masks):
    ...
    def _assemble_tokens(self, input_task_tokens, ids_keep, ids_restore):
    ...
    def _add_global_tokens(self, input_tokens, B):
        global_tokens = repeat(self.global_tokens, '() n d -> b n d', b=B)
        input_tokens = torch.cat([input_tokens, global_tokens], dim=1)
        return input_tokens

    def _forward_encoder(self, input_tokens):
        return self.encoder(input_tokens)

    def _forward_output_adapters(self, encoder_tokens, input_info, ids_keep, ids_restore, fp32_output_adapters):
    ...
    def forward(self,
        ...
        x, B, H, W = self._prepare_input(x)
        input_task_tokens = self._encode_input(x)
        input_info = self.generate_input_info(input_task_tokens=input_task_tokens, image_size=(H, W))
        if mask_inputs:
            if task_masks is None:
                task_masks, ids_keep, ids_restore = self._mask_tokens(input_task_tokens, num_encoded_tokens, alphas, sample_tasks_uniformly)
            else:
                ids_keep, ids_restore = self._handle_predefined_masks(input_task_tokens, task_masks)
            input_tokens = self._assemble_tokens(input_task_tokens, ids_keep, ids_restore)
        else:
            ids_keep = torch.arange(sum([tensor.shape[1] for tensor in input_task_tokens.values()]), device=list(input_task_tokens.values())[0].device)
            ids_restore = ids_keep
            input_tokens = torch.cat([task_tokens for task_tokens in input_task_tokens.values()], dim=1)
        input_tokens = self._add_global_tokens(input_tokens, B)
        encoder_tokens = self._forward_encoder(input_tokens)
        preds, task_masks = self._forward_output_adapters(encoder_tokens, input_info, ids_keep, ids_restore, fp32_output_adapters)
