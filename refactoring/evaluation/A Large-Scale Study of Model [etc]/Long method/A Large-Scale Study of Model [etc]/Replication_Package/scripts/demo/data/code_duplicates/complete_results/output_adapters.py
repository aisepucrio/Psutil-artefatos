...
class SpatialOutputAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        self._init_task_embeddings(context_tasks)
        self._init_decoder(depth, mlp_ratio, num_heads, qkv_bias, drop_rate, attn_drop_rate, drop_path_rate, norm_layer)
        self.out_proj = nn.Linear(self.dim_tokens, self.num_channels * self.P_H * self.P_W)
        if self.dim_tokens_enc is not None:
            self.init(dim_tokens_enc=dim_tokens_enc)

    def _init_positional_encoding(self):
        self.P_H = max(1, self.patch_size_full[0] // self.stride_level)
        self.P_W = max(1, self.patch_size_full[1] // self.stride_level)
    ...
    def _init_task_embeddings(self, context_tasks):
    ...
    def _init_decoder(self, depth, mlp_ratio, num_heads, qkv_bias, drop_rate, attn_drop_rate, drop_path_rate, norm_layer):
        if self.use_xattn:
    ...
    def _get_queries(self, context_with_mask, input_info, N_H, N_W, B):
    ...
    def get_queries_and_context(self, context_tokens, input_info, ids_keep, ids_restore):
        ...
        context_tokens_without_global = self._remove_global_tokens(context_tokens, input_info)
        context_with_mask = self._add_mask_tokens_and_unshuffle(context_tokens_without_global, input_info, ids_restore, B)
        ...
        queries = self._get_queries(context_with_mask, input_info, N_H, N_W, B)
        ...
        context_tokens = self._add_global_tokens_back(context_tokens_without_global, context_tokens, input_info)
        return queries, context_tokens

    def _remove_global_tokens(self, context_tokens, input_info):
        if 'num_global_tokens' in input_info:
            return context_tokens[:, :-input_info['num_global_tokens']]
        return context_tokens

    def _add_mask_tokens_and_unshuffle(self, context_tokens_without_global, input_info, ids_restore, B):
    ...
    def _add_global_tokens_back(self, context_tokens_without_global, context_tokens, input_info):
        if 'num_global_tokens' in input_info:
    ...
    def forward(self,
        ...
        x = self._perform_cross_attention_and_mlp(queries, context_tokens)
    ...
    def _perform_cross_attention_and_mlp(self, queries, context_tokens):
        if self.use_xattn:
...
class LinearOutputAdapter(nn.Module):
    ...
    def _init_head(self):
        ...
        self.head = nn.Linear(self.dim_tokens_enc, self.num_classes) if self.num_classes > 0 else nn.Identity()
...
class DPTOutputAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        self.head = self._create_head(feature_dim, use_bn)
        if self.dim_tokens_enc is not None:
            self.init(dim_tokens_enc=dim_tokens_enc)

    def _create_head(self, feature_dim, use_bn):
